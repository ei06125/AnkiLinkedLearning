...
Manual acceptance: load `OutDir/extension/` in Chrome, capture an authenticated Japanese LinkedIn Learning transcript, switch between the Words and Individual kanji tabs, confirm only one pane and target are active, select each target type, and check the corresponding LinkedIn transcript highlight. Confirm the capture disappears when the panel session ends while cards remain. Verify notices disappear after five seconds. Review automatic readings and translations, preview, export, and import into Anki Desktop. Real Anki import remains a manual check.
